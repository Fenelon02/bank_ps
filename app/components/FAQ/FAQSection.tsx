            ...
            {bankAccountFAQ.map((faq) => {
                ...
                return (
                    <div key={faq.id} className="w-full max-w-2xl p-2 flex flex-col items-center">
                        <Button additionalClass={`font-semibold w-full ${open === faq.id ? "border-blue-600 border-b-transparent rounded-b-none" : "rounded-lg"}`} variant="outline" size="large" onClick={() => setOpen(open === faq.id ? null : faq.id)}>{faq.question} </Button>
